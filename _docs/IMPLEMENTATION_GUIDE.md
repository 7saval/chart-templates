...
export function KpiCard({ data, isLoading, error }: KpiCardProps) {
  ...
  if (error) {
    return (
      <Card className="bg-card border-border">
        <CardContent className="pt-6 text-sm text-status-critical">
          {error}
        </CardContent>
      </Card>
    );
...
export const WithWarning: Story = {
  args: { data: { ...baseData, status: "warning" } },
};
export const Loading: Story = { args: { data: baseData, isLoading: true } };
export const Empty: Story = {
  args: { data: { ...baseData, trend: undefined } },
};
```
...
export function TrendLineChart({
  ...
  if (error)
    return (
      <div
        className="flex items-center justify-center text-sm text-status-critical"
        style={{ height }}
      >
        {error}
      </div>
...
export function BarChart({
  ...
  const option = {
    ...DL_OPS_DARK_THEME,
    grid: {
      left: orientation === "horizontal" ? 80 : 40,
      right: 20,
      top: 16,
      bottom: 24,
    },
...
const series = [
  {
    name: "Producer",
    data: xLabels.map((l, i) => ({ label: l, value: 100 + i * 5 })),
    color: "#3b82f6",
  },
  {
    name: "Consumer",
    data: xLabels.map((l, i) => ({ label: l, value: 90 + i * 4 })),
    color: "#22c55e",
  },
];
...
export const WithWarning: Story = {
  args: { ...Default.args, error: undefined },
};
...
export function TrendLineChartRecharts({
  series,
  xLabels,
  height = 240,
  isLoading,
  ...
  if (isLoading)
    return (
      <div style={{ height }} className="animate-pulse bg-muted/20 rounded" />
    );
...
export function DonutRingChartRecharts({
  data,
  height = 240,
}: DonutRingChartProps) {
  return (
...
### 4-0. 설계 검토 — 왜 `StatusDataTable`만 `@tanstack/react-table`을 쓰는가

이 Phase에서 만드는 두 테이블은 겉보기엔 비슷하지만 성격이 다릅니다. 그 차이가 라이브러리 선택 기준입니다.

|                 | `StatusDataTable` (4-1)                                                                                                                | `AlertEventTable` (4-2)                                    |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| 타입            | 제네릭 `<T extends Record<string, unknown>>`                                                                                           | 고정 타입 `AlertEvent[]`                                   |
| 컬럼            | 소비자가 `TableColumn<T>[]`로 런타임에 정의 — 브로커, 워커, 파티션 등 화면마다 컬럼 구성과 정렬 가능 여부(`sortable?: boolean`)가 다름 | 컴포넌트 내부에 Time/Severity/Message/Target/Status로 고정 |
| 필요한 상호작용 | 컬럼마다 다른 정렬 가능 여부, 다양한 데이터 셰이프에 재사용                                                                            | 심각도 탭 필터 하나만                                      |

**결정:**

- **`StatusDataTable` → `@tanstack/react-table` 도입.** 컬럼 정의를 데이터와 분리해서 여러 화면(브로커, 워커, 노드 등)에 재사용하는 범용 컴포넌트이므로, `columnHelper.accessor`로 컬럼을 선언적으로 정의하고 `getSortedRowModel` + `enableSorting`으로 컬럼별 정렬을 라이브러리에 위임합니다. 직접 구현하면 컬럼마다 비교 함수를 따로 작성해야 하는데, react-table을 쓰면 이 로직이 필요 없습니다.
- **`AlertEventTable` → 순수 shadcn `Table` 유지.** 도메인이 알림 이벤트로 고정된 단일 목적 컴포넌트라 컬럼도 필터 로직도 하드코딩으로 충분합니다. 정렬 요구사항도 없이 심각도 탭 필터 하나뿐이라 `useState` + `Array.filter`로 끝나며, react-table의 컬럼 정의 시스템을 들일 이유가 없습니다.
- 기준: **제네릭·재사용 컴포넌트는 react-table, 도메인 특화 고정 컴포넌트는 순수 shadcn.** `AlertEventTable`에 정렬이 필요해지면(예: 헤더 클릭 정렬), 컬럼이 고정된 상태로 남는다면 `useState` 기반 수동 정렬을 우선 검토하고, 컬럼 구성이 가변적으로 바뀔 조짐이 보이면 그때 react-table 도입을 재검토합니다.
          ...
          {filtered.map((e) => (
            <TableRow key={e.id}>
              ...
              {showAckColumn && (
                <TableCell>
                  <Badge
                    variant={e.status === "ack" ? "secondary" : "destructive"}
                  >
                    {e.status ?? "unack"}
                  </Badge>
                </TableCell>
...
```tsx
import {
  ReactFlow,      // React Flow 캔버스 자체를 렌더링하는 메인 컴포넌트
  Background,     // 캔버스 배경에 점/격자 패턴을 그려주는 보조 컴포넌트
  Controls,       // 확대/축소, fitView 등 조작 UI(줌 컨트롤 박스)를 그려주는 컴포넌트
  Handle,         // 노드에서 엣지(선)가 연결되는 "연결점"을 표시하는 컴포넌트
  Position,       // Handle의 위치(Top/Bottom/Left/Right)를 지정하는 enum
} from "@xyflow/react";
import type { NodeProps } from "@xyflow/react";
// 커스텀 노드 컴포넌트가 React Flow로부터 받는 props의 타입 (data, id, selected 등)
import "@xyflow/react/dist/style.css";
// xyflow 라이브러리의 기본 캔버스/컨트롤/핸들 스타일을 불러옴 (이거 없으면 레이아웃이 깨짐)
import { PipelineFlowNode } from "@/components/flow/PipelineFlowNode";
// 이미 만들어둔 노드 UI(카드형 컴포넌트)를 재사용
import type {
  PipelineFlowDiagramProps,  // PipelineFlowDiagram 컴포넌트가 받을 props 타입 (nodes, edges, height)
  PipelineFlowNodeType,      // xyflow의 Node<T> 제네릭으로 만든 노드 데이터 타입
} from "./PipelineFlowDiagram.types";

// React Flow가 "pipelineNode" 타입 노드를 만날 때 실제로 무엇을 렌더링할지 정의하는 래퍼 컴포넌트
function FlowNodeRenderer({ data }: NodeProps<PipelineFlowNodeType>) {
  // data: 이 노드에 연결된 실제 파이프라인 정보(name, metrics, status 등)
  return (
    <>
      <Handle
        type="target"          // 이 핸들은 "다른 노드로부터 들어오는" 연결선을 받는 지점
        position={Position.Top} // 노드 위쪽에 위치
        className="border-0! bg-transparent!"
        // 기본 핸들 스타일(테두리/배경)을 지워서 시각적으로 안 보이게 처리 (연결 기능만 유지)
      />
      <PipelineFlowNode {...data} />
      {/* 실제 눈에 보이는 노드 UI. data를 그대로 펼쳐서 PipelineFlowNode의 props로 전달 */}
      <Handle
        type="source"             // 이 핸들은 "다른 노드로 나가는" 연결선을 시작하는 지점
        position={Position.Bottom} // 노드 아래쪽에 위치
        className="border-0! bg-transparent!"
        // 위와 동일하게 시각적으로만 숨김
      />
    </>
...
// React Flow에게 "pipelineNode"라는 타입 문자열이 나오면 FlowNodeRenderer로 그리라고 매핑
// (컴포넌트 바깥에 선언하는 이유: 렌더링마다 새 객체가 생성되면 xyflow가 리렌더 경고를 띄우기 때문)
const nodeTypes = { pipelineNode: FlowNodeRenderer };

export function PipelineFlowDiagram({
  nodes,          // 그릴 노드 배열 (파이프라인 각 단계)
  edges,          // 그릴 엣지 배열 (단계 간 연결선)
  height = 360,   // 캔버스 높이, 기본값 360px
}: PipelineFlowDiagramProps) {
  // 원본 edges에 스타일/애니메이션 속성을 덧입힌 새 배열을 만듦 (원본은 건드리지 않음)
  const styledEdges = edges.map((e) => ({
    ...e,                                    // 기존 엣지 속성(id, source, target, data 등)은 그대로 유지
    type: "smoothstep",                      // 엣지 선 모양을 계단형 곡선(smoothstep)으로 지정
    animated: e.data?.isBottleneck === true, // 병목 구간이면 선을 움직이는 애니메이션으로 강조
    style: {
      stroke: e.data?.isBottleneck ? "#ef4444" : "#334155",
      // 병목이면 빨간색, 아니면 기본 슬레이트 색으로 선 색상 지정
      strokeWidth: 2,                         // 선 두께 2px
      strokeDasharray: e.data?.isBottleneck ? "4 3" : undefined,
      // 병목이면 점선(4px 선-3px 공백 반복), 아니면 실선
    },
    labelStyle: { fill: "#94a3b8", fontSize: 10 },
    // 엣지에 라벨(텍스트)이 있을 경우 글자 색/크기 지정
    labelBgStyle: { fill: "#1e293b" },
    // 라벨 뒤 배경 박스 색상 지정 (가독성을 위한 배경)
  }));

  return (
    <div style={{ height }}>
      {/* ReactFlow는 부모 요소의 크기를 기준으로 캔버스를 그리므로 높이를 명시적으로 지정해야 함 */}
      <ReactFlow
        nodes={nodes}                 // 위에서 받은 노드 배열 그대로 전달
        edges={styledEdges}           // 스타일을 입힌 엣지 배열 전달
        nodeTypes={nodeTypes}         // "pipelineNode" 타입을 FlowNodeRenderer로 렌더링하도록 등록
        nodesDraggable={false}        // 노드를 마우스로 드래그해서 옮기지 못하게 막음 (읽기 전용 다이어그램)
        nodesConnectable={false}      // 사용자가 직접 새 연결선을 만들지 못하게 막음
        elementsSelectable={false}    // 노드/엣지를 클릭해서 선택하는 기능을 막음
        panOnScroll                   // 스크롤로 캔버스를 이동(팬) 가능하게 함
        zoomOnScroll={false}          // 스크롤로 확대/축소는 하지 않도록 막음 (팬과 충돌 방지)
        proOptions={{ hideAttribution: true }}
        // React Flow 우측 하단에 뜨는 "React Flow" 워터마크/링크를 숨김
        fitView
        // 초기 렌더링 시 모든 노드가 화면에 들어오도록 자동으로 줌/위치 조정
      >
        <Background color="#1e293b" gap={24} />
        {/* 캔버스 배경에 24px 간격의 점 패턴을 어두운 슬레이트 색으로 표시 */}
        <Controls showInteractive={false} />
        {/* 줌 인/아웃/fitView 버튼을 표시하되, "상호작용 잠금 토글" 버튼은 숨김 */}
      </ReactFlow>
    </div>
  );
}
  ...
  series: [
    {
      name: "Latency",
      color: "#3b82f6",
      data: [4.5, 4.3, 4.6, 4.2, 4.2].map((v, i) => ({
        label: String(i),
        value: v,
      })),
    },
...
import {
  kafkaKpis,
  kafkaBrokerTopology,
  kafkaConsumerLagColumns,
  kafkaConsumerLagRows,
  ...
  return (
    <div className="space-y-4">
      ...
      <SectionPanel title="Consumer Lag by Topic">
        <StatusDataTable
          columns={kafkaConsumerLagColumns}
          data={kafkaConsumerLagRows}
        />
      </SectionPanel>
    </div>
  );
